...
for it is limited.  And with those limitations come challenges. All sorts of
    different challenges and limitations face the development and production of
    AR apps. Here are some that we considered when working on our app.
...
screen in Figure~{fig:adobeXdDesign}. Before, we worked under the assumption
...
the growing code complexity more manageable. Excluding Node modules, the only
...
Atticus took photographs of various signs throughout Kinkakuji for us to use as
image targets. In this design, the AR camera superimposes an "info" button on
top of the sign. Tapping on this button brings up a separate UI element
containing the translation, which can be seen in Figure~\ref{fig:signTranslation}.
...
ordering*
transcripts*
no more ground plane detection*
many, many image targets
thumbnails for the chapter
