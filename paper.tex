\documentclass[8pt, american, titlepage]{article}
...
\geometry{paperwidth=340mm, paperheight=16383pt, marginparsep=20pt, marginparwidth=100pt, textheight=16263pt, footskip=40pt}
...
% TODO is starting with "A special thank you" a complete clause?
...
\begin{figure}[p]
	\centering
	\includegraphics[width=0.2\textwidth]{ar-example.jpg}
...
\begin{figure}[p]
	\centering
	\includegraphics[width=0.2\textwidth]{kinkakuji-1.jpg}
...
\begin{figure}[p]
	\centering
	\includegraphics[width=0.2\textwidth]{ar-marker.jpg}
...
\begin{figure}[p]
	\centering
	\includegraphics[width=0.2\textwidth]{pokemon-go.jpg}
...
\begin{figure}[p]
	\centering
	\includegraphics[width=0.2\textwidth]{izi-travel.png}
...
\begin{figure}[p]
	\centering
	\includegraphics[width=0.2\textwidth]{kinkakuji.jpg}
...
emulate MS-DOS, the editor responded ``Yes, it's possible for a 64 to emulate an
IBM PC, in the same sense that it's possible to bail out Lake Michigan with a
teaspoon.'' The editor goes on to explain ``Emulation is a complex business, but
here's one rule of thumb: The only way to successfully emulate a machine is with
a much more powerful machine''~\autocite{warick1988}. In our case, emulating
Android on a desktop operating system is closer to emulating a C64 on an IBM PC
running MS-DOS (rather than the other way around, thankfully). The takeaway is
that the speed problem facing emulators is just as real as it was in 1988.}
Considering that AR already pushes modern phone hardware to the limit, emulation
...
\begin{figure}[p]
	\centering
	\includegraphics[width=0.2\textwidth]{kinkakuji-grounds.jpg}
...
\begin{figure}[p]
	\centering
	\includegraphics[width=0.2\textwidth]{updated-editour.png}
...
\begin{figure}[p]
	\centering
	\includegraphics[width=0.2\textwidth]{welcome-card-editour.png}
...
\begin{figure}[p]
	\centering
	\includegraphics[width=0.2\textwidth]{drawing-region-editour.png}
...
\begin{figure}[p]
	\centering
	\includegraphics[width=0.2\textwidth]{rename-subcard-editour.png}
...
\begin{figure}[p]
	\centering
	\includegraphics[width=0.2\textwidth]{media-subcard-editour.png}
...
create a blue pin-shaped marker that the user can also drag. Clicking on the
coordinate box in the subcard will also create this blue pin-shaped marker; the
map will also pan to this coordinate. The user can also delete vertices in the
same way that files can be deleted from the ``Media'' subcard by clicking on the
`X' button on that vertex's coordinate card. If there are exactly three points
in a region, the `X' buttons will be ``grayed-out,'' preventing the user from
deleting any more points, since a polygon must consist of at least three points.

\begin{figure}[p]
	\centering
    \includegraphics[width=0.2\textwidth]{edit-subcard-with-map-editour.png}
...
\begin{figure}[p]
	\centering
    \includegraphics[width=0.2\textwidth]{delete-subcard-editour.png}
...
\begin{figure}[p]
	\centering
    \includegraphics[width=0.2\textwidth]{uploading-message-editour.png}
...
\begin{figure}[p]
	\centering
    \includegraphics[width=0.2\textwidth]{download-card-editour.png}
...
\begin{figure}[p]
	\centering
    \includegraphics[width=0.2\textwidth]{jump-card-editour.png}
...
Figure~\ref{fig:kanbanBoard} in the Appendix).

\begin{figure}[p]
	\centering
	\includegraphics[width=0.2\textwidth]{super-famicom-controller.jpg}
...
The classes within CSS were designed so they could be reused
easily and composed with one another. If, for example, we needed to change the
font of every element that was styled like a button, we could do so by simply
changing the \texttt{font-family} attribute in a single CSS class. Consider the
green bar pictured in Figure~\ref{fig:mediaSubcard}. Multiple CSS classes were
composed in order to inherit the visual descriptions of other UI elements. The
four CSS classes on this bar are \texttt{button}, \texttt{greenbutton},
...
the \texttt{button} class and either \texttt{greenbutton}, \texttt{orangebutton}
or \texttt{bluebutton} will be styled like a button and have a new color palette
applied. In order to differentiate the buttons that reveal subcards, the
\texttt{outlinebutton} class is used. This class takes the custom
\texttt{--bg-color} property and applies it to the border, drop shadow and text
color instead of the background. This class was also applied to the buttons that
can be seen in Figures~\ref{fig:jumpCard}~and~\ref{fig:downloadCard}. Finally,
the \texttt{revealer} class changes the text from centered to left-justified
among other minor formatting adjustments. CSS allowed us to define the look and
feel programmatically with the need to create graphical assets using image
editing software. This means that anyone using the codebase of Editour for
different purposes can easily change the look and feel of the frontend by
changing the CSS.\footnote{This is allowed since Editour is free software, which
is discussed briefly in Section~\ref{sec:futureOfEditour}.}
...
% TODO explain what stateless operations are
...
systemd service to run Editour as a daemon and keep its log files organized. The
Node.js backend was served by an Nginx reverse proxy running on the same virtual
machine.
...
free license, namely the GNU General Public License. This allows anyone to
expand the frontend and backend to suit their needs. With additional hardware
and by extending the current implementation of both the frontend and backend,
Editour could be used as the groundwork to create an entire crowd-sourced
walking tour platform, allowing users to design tours for others to take.
...
% TODO add a picture that shows how having only circular regions limits possible
% tour designs
...
poor results. Google~Translate's text recognition and machine-learning powered
...
``scan'' mode, the user first takes a photo. This will sometimes produce
useless results, as seen in Figure~\ref{fig:translateInstant}. Then, the user
is prompted to highlight where text appears in the photo by dragging lines onto
the screen where text appears, as seen in Figure~\ref{fig:translateScan}. After
the translation is complete, the translated text appears in a separate UI
element, which can be seen in Figure~\ref{fig:translateFinalCard}. In most
cases, it is more convenient and readable to have the translation appear in a
separate scrollable text box. This motivated the current design of the ``info
buttons'' in ARuko. Instead of placing text directly onto signage, we place a
blue button with a stylized ``\textit{i}'' in the center. By tapping on this
button, a separate UI element will appear containing the translated text. Since
this button is cast into the world using the AR camera, the button appears to
be placed directly on top of the content.

% TODO make it so these images don't appear so far away (thank you LaTeX)
\begin{figure}[p]
	\centering
	\includegraphics[width=0.2\textwidth]{translate-instant.jpg}
...
\begin{figure}[p]
	\centering
	\includegraphics[width=0.2\textwidth]{translate-scan.jpg}
...
\begin{figure}[p]
	\centering
	\includegraphics[width=0.2\textwidth]{translate-final-card.png}
	\caption{The final page seen after using ``scan'' mode in Google Translate}
	\label{fig:translateFinalCard}
\end{figure}

Additionally, we would not try to use any text recognition or translation APIs
to automate the translation. Figure~\ref{fig:translateInstant} demonstrates
that this kind of technology needs to mature before it is completely usable,
especially for Japanese. Instead, we had Atticus share a Google Sheets
spreadsheet. Hikaru Inoue manually translated each sign and provided notes about
his decision making process about each translation. Once this time-intensive
labor was completed, we took each field of translated text from the spreadsheet
and implemented it directly into the app, associating each translation with the
correct image target.
...
\begin{figure}[p]
	\centering
    \includegraphics[width=0.2\textwidth]{ar-map-feature.png}
...
\begin{figure}[p] \centering
    \includegraphics[width=0.2\textwidth]{kinkakuji-ticket.jpeg}
...
\begin{figure}[p]
	\centering
	\includegraphics[width=0.2\textwidth]{souvenir.png}
...
\begin{figure}[p]
	\centering
	\includegraphics[width=0.2\textwidth]{kinkakuji-horizontal.png}
...
\begin{figure}[p]
	\centering
	\includegraphics[width=0.2\textwidth]{old-ui.png}
...
\begin{figure}[p]
	\centering
	\includegraphics[width=0.2\textwidth]{floating-cube.png}
...
To get the smooth open/close animation, we ``lerp'' (linearly interpolate)
the menu to the corresponding position each frame until it is fully opened or
...
learned about how audio sliders work in Unity with this feature, and would
...
\begin{figure}[p]
	\centering
	\includegraphics[width=0.2\textwidth]{adobe-xd-design.png}
...
% "scrollable" is for sure a neologism but let's normalize it through usage
...
\begin{figure}[p]
	\centering
	\includegraphics[width=0.2\textwidth]{editour-additions.png}
...
screen on Figure~\ref{fig:adobeXdDesign} and Figure~\ref{fig:infoZoomed} shows a
mock-up of this feature. Atticus took photographs of various signs throughout
...
seen in Figure~\ref{fig:translationZoomed}. The details of this feature is
...
stylized incorrectly with ``AdobeXd'' in the description of the Unity
package. This is also different from ``Adobe Xd'', the way in which it is
stylized in the title.} We suspect this sounds too good to be true because it
is. As of September 20\textsuperscript{th}, 2019, there are 6 user reviews. All
of the five-star reviews were have the caveat ``Reviewer was gifted package by
publisher'' which which we interpreted as a warning sign. Because of this, we
...
\begin{figure}[p] \centering
    \includegraphics[width=0.2\textwidth]{chapters-zoomed.png}
    \caption{Larger screenshot of the chapter selection screen mock-up}
    \label{fig:chaptersZoomed}
\end{figure}

\begin{figure}[p] \centering
    \includegraphics[width=0.2\textwidth]{gallery-zoomed.png}
    \caption{Larger screenshot of the transcript and image gallery mock-up}
    \label{fig:galleryZoomed}
\end{figure}

\begin{figure}[p] \centering
    \includegraphics[width=0.2\textwidth]{info-zoomed.png}
    \caption{Larger screenshot of the info button mock-up}
    \label{fig:infoZoomed}
\end{figure}

\begin{figure}[p] \centering
    \includegraphics[width=0.2\textwidth]{map-zoomed.png}
    \caption{Larger screenshot of the route screen mock-up}
    \label{fig:mapZoomed}
\end{figure}

\begin{figure}[p] \centering
    \includegraphics[width=0.2\textwidth]{translation-zoomed.png}
    \caption{Larger screenshot of the translation window mock-up}
    \label{fig:translationZoomed}
\end{figure}
...
% TODO maybe include an image of pocket mode here
...
\begin{figure}[p]
	\centering
	\includegraphics[width=0.2\textwidth]{issues-page-example.png}
...
\begin{figure}[p]
	\centering
	\includegraphics[width=0.2\textwidth]{intersections-diagram.png}
...
Editour was flexible enough to easily swap out media and add regions to the
tour, Editour proved itself to be as helpful for testing as it was for
...
\begin{figure}[p]
	\centering
	\includegraphics[width=0.2\textwidth]{ritsu-tour.png}
...
\begin{figure}[p]
	\centering
	\includegraphics[width=0.2\textwidth]{kinkakuji-tour.png}
...
\begin{figure}[p]
	\centering
	\includegraphics[width=0.2\textwidth]{kinkakuji-chunkier-tour.png}
...
\begin{figure}[p]
	\centering
	\includegraphics[width=0.2\textwidth]{four-i-buttons.png}
...
\begin{figure}[p]
	\centering
	\includegraphics[width=0.2\textwidth]{galaxy-s10+.jpg}
...
\begin{figure}[p]
	\centering
	\includegraphics[width=0.2\textwidth]{iphone-x.jpg}
...
\begin{figure}[p]
	\centering
	\includegraphics[width=0.2\textwidth]{exhibition-scan.png}
...
interested in triggering 3D videos through AR. As the scope of the project 
...
for this feature, preventing us from spending time developing it. 
...
experience building web applications with HTML, CSS, and JavaScript, and Editour
...
project, we were able polish the app enough that it would be feasible for
Atticus to eventually sell it. The UI could use more testing with real
...
\begin{figure}[p]
	\centering
	\includegraphics[width=0.2\textwidth]{kanban-board.png}
...
pull requests would contain the issues they were resolving. Secondly, creating a
...
related to biology, and helped us understand the mathematical model we would
...
\begin{figure}[p] \centering
    \includegraphics[width=0.2\textwidth]{reference-model.png}
...
\begin{figure}[p] \centering
    \includegraphics[width=0.2\textwidth]{interactive-model.png}
...
\begin{figure}[p] \centering
    \includegraphics[width=0.2\textwidth]{heart-gui.png}
...
version we were using, and updated the version of Vuforia to the most recent
...
\begin{figure}[p]
	\centering
	\includegraphics[width=0.2\textwidth]{no-ar-working.jpg}
